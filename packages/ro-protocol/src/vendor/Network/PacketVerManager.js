...
function parseCharList(fp, end) {
	...
	if (!blockSize || length % blockSize) {
		...
		// game-project: no roBrowser isto abria um error box da UI. Aqui roda
		// no gateway (Node, sem DOM), entao so registra e devolve vazio — quem
		// chama decide o que fazer com a lista de personagens vazia.
		if (matches.length !== 1) {
			console.error(
				'[ro-protocol] CHARACTER_INFO size error, auto-detect falhou. blockSize: "' +
					blockSize +
					'", list length: ' +
					length
			);
...
export default {
	// Get Back data
	get value() {
		return _value > 0 ? _value : globalThis.ROConfig?.servers?.[0]?.packetver || globalThis.ROConfig?.packetver || 0;
	},
