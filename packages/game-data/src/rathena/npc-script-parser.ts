import type { Expr, Statement, EntryPoint, NpcScriptAst } from "./npc-script-ast";
import { tokenize, type Token } from "./npc-script-lexer";
...
function parseStatementInner(s: TokenStream): Statement {
  ...
  if (s.isIdent("if")) {
    s.advance();
    s.expectPunct("(");
    const testStart = s.peek().start;
    const test = parseAssign(s);
    const testEnd = s.tokens[s.pos - 1]!.end;
    s.expectPunct(")");
    // logo depois do `{` (forma com bloco) ou onde o statement único começa
    // (forma sem chaves) — pro Writer recortar um comentário/espaço que
    // vier ANTES do primeiro statement de verdade do ramo, em vez de
    // ancorar no início do PRIMEIRO STATEMENT e perder esse vão.
    const consequentBodyStart = s.isPunct("{") ? s.peek().end : s.peek().start;
    const consequent = parseBlockOrSingle(s);
    let alternate: Statement[] | null = null;
    let alternateBodyStart: number | undefined;
    if (s.isIdent("else")) {
      s.advance();
      alternateBodyStart = s.isPunct("{") ? s.peek().end : s.peek().start;
      alternate = parseBlockOrSingle(s);
    }
    return { kind: "IfStatement", test, consequent, alternate, line, testStart, testEnd, consequentBodyStart, alternateBodyStart };
  }
  ...
  if (s.isIdent("switch")) {
    s.advance();
    s.expectPunct("(");
    const discriminantStart = s.peek().start;
    const discriminant = parseAssign(s);
    const discriminantEnd = s.tokens[s.pos - 1]!.end;
    s.expectPunct(")");
    s.expectPunct("{");
    const cases: { test: Expr | null; body: Statement[]; headerStart?: number; headerEnd?: number }[] = [];
    while (!s.isPunct("}") && s.peek().type !== "EOF") {
      const headerStart = s.peek().start;
      ...
      s.expectPunct(":");
      const headerEnd = s.tokens[s.pos - 1]!.end;
      const body: Statement[] = [];
      while (!s.isIdent("case") && !s.isIdent("default") && !s.isPunct("}") && s.peek().type !== "EOF") {
        body.push(parseStatement(s));
      }
      cases.push({ test, body, headerStart, headerEnd });
    }
    s.expectPunct("}");
    return { kind: "SwitchStatement", discriminant, cases, line, discriminantStart, discriminantEnd };
  }
...
function splitEntryPointTokens(
  tokens: Token[],
): { label: string | null; line: number; bodyStart: number; slice: Token[] }[] {
  const segments: { label: string | null; line: number; bodyStart: number; slice: Token[] }[] = [];
  let depth = 0;
  ...
  let start = 0;
  // corpo PRINCIPAL sempre começa em 0 (início do texto do NPC inteiro) —
  // pro Writer poder reusar um comentário/espaço que vem ANTES do primeiro
  // statement de verdade, em vez de perder ele (achado: `bodyStart` como
  // "início do primeiro TOKEN" perdia qualquer coisa entre `{`/`:` e o
  // primeiro statement — leia1.txt 2026-08-09, "preserve comentários
  // quando possível").
  ...
  while (i < tokens.length) {
    ...
    if (t.type === "Punct" && t.value === "{") depth++;
      ...
      currentLine = t.line;
      i += 2; // pula "Nome" ":"
      currentBodyStart = tokens[i - 1]!.end; // logo depois do ":" — mesmo raciocínio do `headerEnd` de case/default
      start = i;
      continue;
    }
    i++;
...
export function parseNpcScript(source: string): NpcScriptAst {
  ...
  const entryPoints: EntryPoint[] = segments.map((seg) => {
    ...
    // `seg.bodyStart` é logo depois do ":" do rótulo (ou 0 pro corpo
    // principal), não o início do primeiro STATEMENT — assim um comentário
    // entre o rótulo e o primeiro statement sobrevive num recorte
    // verbatim, em vez de cair num vão que ninguém reconstrói. `seg.slice`
    // sempre termina num EOF sintético (`splitEntryPointTokens`), então o
    // último token DE VERDADE é o penúltimo. Corpo vazio (`OnInit:` seguido
    // de outro rótulo) ainda tem `bodyStart` válido (não há o que perder;
    // não muda nada, já que uma lista de unidades vazia nem consulta ele).
    const real = seg.slice.slice(0, -1);
    const bodyStart = seg.bodyStart;
