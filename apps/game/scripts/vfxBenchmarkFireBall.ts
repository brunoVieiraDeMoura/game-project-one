/**
 * `pnpm --filter @ragnarok/game vfx:benchmark:fireball` (ou
 * `VFX_BENCH_HEADED=1 npx tsx scripts/vfxBenchmarkFireBall.ts` direto de
 * `apps/game`) — benchmark DEDICADO de Fire Ball (reconstrução
 * 2026-08-19-v), MESMO padrão de `vfxBenchmarkLightBolt.ts` (ver aquele
 * arquivo pro raciocínio completo, não duplicado aqui).
 *
 * Porta PRÓPRIA (3095, nunca 3096/3097/3098/3099 — Light Bolt/Cold Bolt/
 * Fire Lance/benchmark principal).
 *
 * Diferente de Fire Lance/Cold Bolt/Eletrocutar: Fire Ball NÃO tem driver
 * per-hit (não é multi-hit, `HitCount:1` real). O caminho de produção é o
 * despacho GENÉRICO (`bindSkillVfx`+`migratedVfxBridge`, ver
 * `fireBallRenderMode.ts`) — exercitado aqui via `__vfxBench.spawnAll(idx)`,
 * a MESMA chamada que `/vfx-bench`'s outros cenários sem driver (Soul
 * Strike/Frost Diver/Stone Curse) já usam, e `__fireBallRenderBench.
 * setTier(tier)` decide qual receita tier-específica está registrada sob o
 * id ESTÁVEL `fireball_impact`/`fireball_cast` no momento do spawn.
 */
...
/** cobre voo (`FIREBALL_FLIGHT_MS=480`) + impacto/AoE inteiro (`IMPACT_TAIL_MS=700`)
 * com folga — a instância inteira (projétil+trail+impacto) morre bem antes disso. */
const MEASURE_WINDOW_MS = 1500;
/** pico visual: pouco depois do impacto (flash+explosão no auge). */
const PEAK_SAMPLE_DELAY_MS = 500;

interface FireBallResult {
  scenario: Scenario;
  players: number;
  perf: PerfSnapshot;
  cullPeak: CullStats;
...
async function measure(page: Page, cdp: CDPSession, players: number, scenario: Scenario, impactIdx: number, castIdx: number): Promise<FireBallResult> {
  ...
  if (scenario !== "baseline") {
    await page.evaluate(
      (t) => (window as unknown as { __fireBallRenderBench: { setTier: (t: Tier) => void } }).__fireBallRenderBench.setTier(t),
      scenario,
    );
    await page.evaluate(
...
function fmt(r: FireBallResult): string {
...
async function main(): Promise<void> {
  ...
  console.log(`[fire-ball-bench] subindo vite dev em :${PORT}... (headless=${HEADLESS})`);
  ...
  try {
    ...
    console.log("[fire-ball-bench] abrindo /vfx-bench...");
    ...
    const impactIdx = scenarios.findIndex((s) => s.aegisName === "MG_FIREBALL" && s.kind === "impact");
    const castIdx = scenarios.findIndex((s) => s.aegisName === "MG_FIREBALL" && s.kind === "cast");
    if (impactIdx === -1) throw new Error("cenário 'Fire Ball — impact' não encontrado em BENCH_SKILLS");

    const results: FireBallResult[] = [];
    for (const players of PLAYER_LEVELS) {
      for (const scenario of SCENARIOS) {
        process.stdout.write(`  [${scenario}] ${players} players... `);
        const r = await measure(page, cdp, players, scenario, impactIdx, castIdx);
        results.push(r);
        console.log(fmt(r));
      }
    }
    ...
    const outFile = path.join(OUT_DIR, `fire-ball-${RUN_LABEL}.json`);
    writeFileSync(outFile, JSON.stringify(results, null, 2));
    console.log(`[fire-ball-bench] resultados em ${outFile}`);
  } finally {
    killViteTree(vite);
  }
}
